...
final Map<String, Room> world = {
  'entrance': Room(
    ...
    descriptionPinyin:
        'Nǐ zhàn zài yī gè hēi\'àn yīnsēn de dòngxué rùkǒu. Bīnglěng de fēng cóng shēnchù hūxiào ér chū.',
    descriptionEn:
        'You stand at the mouth of a dark, foreboding cave. Cold wind whistles out from the depths.',
  ...
  'outside': Room(
    ...
    descriptionPinyin:
        'Nǐ huí dào le yángguāng míngmèi de cǎodì shàng. Dòngxué rùkǒu zài nánbiān.',
    descriptionEn:
        'You are back in the sunny meadow. The cave entrance is to the south.',
    exits: {'南': 'entrance'},
  ),
  'hallway': Room(
    id: 'hallway',
    ...
    descriptionPinyin:
        'Yī tiáo yóu cūcāo shítou záo chéng de xiácháng zǒuláng. Dīshuǐ shēng zài yuǎnchù huídàng.',
    descriptionEn:
        'A long, narrow hallway carved from rough stone. Dripping water echoes in the distance.',
  ...
  'armory': Room(
    ...
    descriptionPinyin:
        'Shēngxiù de wǔqì guà zài qiáng shàng. Jiǎoluò lǐ fàng zhe yī gè pòsǔn de tiězhēn.',
    descriptionEn:
        'Rusted weapons hang on the walls. A broken anvil sits in the corner.',
    exits: {'东': 'hallway'},
    item: Item(
      name: '剑',
      namePinyin: 'Jiàn',
      ...
      descriptionPinyin:
          'Yī bǎ shēngxiù dàn réngrán fēnglì de jiàn. (Zhàndòu shānghài +10)',
      descriptionEn:
          'A rusty but still sharp sword. (+10 attack damage in combat)',
    ),
  ),
  'treasury': Room(
    id: 'treasury',
    ...
    descriptionPinyin:
        'Jīnbì hé zhūbǎo sànluò yī dì. Zhōngyāng lì zhe yī gè pedestal.',
    descriptionEn:
        'Gold coins and jewels are scattered across the floor. A pedestal stands in the center.',
    exits: {'西': 'hallway'},
    monster: '巨魔',
    monsterPinyin: 'Jùmó',
    item: Item(
      ...
      descriptionPinyin:
          'Yī gè huálì de jīn bēi, sànfā zhe gǔlǎo de mófǎ guāngmáng. Zhè jiù shì nǐ xúnzhǎo de mùbiāo!',
      descriptionEn:
          'A magnificent golden chalice, glowing with ancient magic. This is what you came for!',
    ),
  ),
};
...
final Map<String, int> monsterHealth = {'哥布林': 40, '巨魔': 80};
...
bool fightMonster(String monsterName, Player player) {
  ...
  while (monsterHp > 0 && player.health > 0) {
    ...
    print(
      'Nǐ de shēngmìng zhí: ${player.health} | $monsterPinyin shēngmìng zhí: $monsterHp',
    );
...
void processCommand(String input, Player player) {
  ...
  if (itemName != null && itemName.isNotEmpty) {
    if (currentRoom.item != null && currentRoom.item!.name == itemName) {
      if (currentRoom.monster != null) {
        print('\n${currentRoom.monster}还在，你无法拿走${currentRoom.item!.name}！');
        print(
          '${currentRoom.monsterPinyin} hái zài, nǐ wúfǎ ná zǒu ${currentRoom.item!.namePinyin}！',
        );
        print(
          'The ${currentRoom.monsterEn()} is still here, you cannot take the ${currentRoom.item!.nameEn()}!',
        );
      } else {
...
void main() {
  ...
  if (Platform.isWindows) {
    ...
    print(''); // This helps initialize the console
  }
  ...
  print(
    '【Zhòngyào】Yídòng mìnglìng géshì: wǎng <fāngxiàng> zǒu huò wǎng <fāngxiàng> qù',
  );
  print(
    '【Important】Movement format: wǎng <direction> zǒu or wǎng <direction> qù',
  );
  print('例如："往北走" 或 "往东去"');
  ...
  while (true) {
    if (player.hasItem('金杯')) {
      ...
      print(
        'You have successfully retrieved the Golden Chalice and escaped the dark caverns.',
      );
    ...
    if (currentRoom.monster != null) {
      ...
      if (defeated) {
        ...
        if (currentRoom.item != null) {
          ...
          print(
            'Guàiwù xiāoshī hòu, nǐ zhùyì dào zhèlǐ yǒu yī gè ${currentRoom.item!.namePinyin}。',
          );
          print(
            'After the monster disappears, you notice a ${currentRoom.item!.nameEn()} here.',
          );
        }
      } else {
        if (player.health > 0) {
